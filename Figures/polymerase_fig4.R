...
skin_counts=t(data.frame(all=as.numeric(read.table("~/Desktop/Phil_POL/PD44594f_all_sig_input.txt")[1,]),
                       gene=as.numeric(read.table("~/Desktop/Phil_POL/PD44594f_all_sig_input_genes.txt")[1,]),
                       cancer_gene=as.numeric(read.table("~/Desktop/Phil_POL/PD44594f_all_sig_input_cabcergenes.txt")[1,])))
...
skin_gene_fit=fit_signatures(counts=skin_counts[2:3,],opportunities = "human-exome",signatures = final_sigs[select,],iter = 20000,warmup = 10000,model="poisson",chains = 2)
skin_gene_fit=fit_signatures(counts=skin_counts[2:3,],signatures = final_sigs[select,],iter = 20000,warmup = 10000,model="poisson",chains = 2)
...
endo_counts=t(data.frame(all=as.numeric(read.table("~/Desktop/Phil_POL/endo_sigs_input_all.txt")[1,]),
                         ...
                         cancer_gene=as.numeric(read.table("~/Desktop/Phil_POL/endo_sigs_input_cancergenes_all.txt")[1,])))
...
endo_gene_fit=fit_signatures(counts=endo_counts[2:3,],opportunities = "human-exome",signatures = final_sigs[select,],iter = 20000,warmup = 10000,model="poisson",chains = 2)
...
final_sigs=t(read.table("~/Desktop/Phil_POL/final_sigs_2020_04_21.txt"))
colon_counts=rbind(read.table("~/Desktop/Phil_POL/colon_sigs_input_all.txt"),
                  read.table("~/Desktop/Phil_POL/colon_sigs_input_genes.txt"),
                  read.table("~/Desktop/Phil_POL/colon_sigs_input_cancergenes.txt"))
...
ratios_colon=matrix(0,nrow=length(patients),ncol=9)
colnames(ratios_colon)=outer(c("all","genes","cancergenes"),c("_mean","_lower95","_upper95"), FUN = "paste0")[1:9]
...
for(patient in patients){
  ...
  fit=fit_signatures(counts=colon_counts[paste0(patient,'_cancergene'),],opportunities = "human-exome",signatures = final_sigs[ref_sigs_present,],iter = 20000,warmup = 10000,model="poisson",chains = 2)
  colon_exposure=retrieve_pars(fit,par = "exposures",hpd_prob = 0.95)
  ratios_colon[patient,"cancergenes_mean"]=rowSums(colon_exposure$mean[1,ref_sigs_present%in%c(normal,pol)])/rowSums(colon_exposure$mean[1,normal])
  ratios_colon[patient,"cancergenes_lower95"]=rowSums(colon_exposure$lower_95[1,ref_sigs_present%in%c(normal,pol)])/rowSums(colon_exposure$lower_95[1,normal])
  ratios_colon[patient,"cancergenes_upper95"]=rowSums(colon_exposure$upper_95[1,ref_sigs_present%in%c(normal,pol)])/rowSums(colon_exposure$upper_95[1,normal])
  print(patient)
}
...
all_counts_POLE=rbind(colon_counts[1,],colSums(colon_counts[paste0(POLE,"_cancergene"),]))
fit=fit_signatures(counts=all_counts_POLE[2,],signatures = final_sigs[c("SBS1","SBS5","SBS10a","SBS10B","SBSA","SBS35like","SBS28"),],opportunities = "human-exome",iter = 20000,warmup = 10000,model="poisson",chains = 2)
colon_exposure=retrieve_pars(fit,par = "exposures",hpd_prob = 0.95)
ratios_colon_geno["POLE","cancergenes_mean"]=rowSums(colon_exposure$mean[1,ref_sigs_present%in%c(normal,pol)])/rowSums(colon_exposure$mean[1,normal])
ratios_colon_geno["POLE","cancergenes_lower95"]=rowSums(colon_exposure$lower_95[1,ref_sigs_present%in%c(normal,pol)])/rowSums(colon_exposure$lower_95[1,normal])
ratios_colon_geno["POLE","cancergenes_upper95"]=rowSums(colon_exposure$upper_95[1,ref_sigs_present%in%c(normal,pol)])/rowSums(colon_exposure$upper_95[1,normal])
...
all_counts_POLD1=rbind(colon_counts[1,],colSums(colon_counts[paste0(POLD1_S478N,"_cancergene"),]))
fit=fit_signatures(counts=all_counts_POLD1[2,],signatures = final_sigs[c("SBS1","SBS5","SBS10C","SBSA","SBS28"),],opportunities = "human-exome",iter = 20000,warmup = 10000,model="poisson",chains = 2)
colon_exposure=retrieve_pars(fit,par = "exposures",hpd_prob = 0.95)
ratios_colon_geno["POLD1_S478N","cancergenes_mean"]=rowSums(colon_exposure$mean[1,ref_sigs_present%in%c(normal,pol)])/rowSums(colon_exposure$mean[1,normal])
ratios_colon_geno["POLD1_S478N","cancergenes_lower95"]=rowSums(colon_exposure$lower_95[1,ref_sigs_present%in%c(normal,pol)])/rowSums(colon_exposure$lower_95[1,normal])
ratios_colon_geno["POLD1_S478N","cancergenes_upper95"]=rowSums(colon_exposure$upper_95[1,ref_sigs_present%in%c(normal,pol)])/rowSums(colon_exposure$upper_95[1,normal])
...
fit=fit_signatures(counts=colon_counts[paste0(patient,'_cancergene'),],opportunities = "human-exome",signatures = final_sigs[ref_sigs_present,],iter = 20000,warmup = 10000,model="poisson",chains = 2)
colon_exposure=retrieve_pars(fit,par = "exposures",hpd_prob = 0.95)
ratios_colon_geno["POLD1_L474P","cancergenes_mean"]=rowSums(colon_exposure$mean[1,ref_sigs_present%in%c(normal,pol)])/rowSums(colon_exposure$mean[1,normal])
ratios_colon_geno["POLD1_L474P","cancergenes_lower95"]=rowSums(colon_exposure$lower_95[1,ref_sigs_present%in%c(normal,pol)])/rowSums(colon_exposure$lower_95[1,normal])
ratios_colon_geno["POLD1_L474P","cancergenes_upper95"]=rowSums(colon_exposure$upper_95[1,ref_sigs_present%in%c(normal,pol)])/rowSums(colon_exposure$upper_95[1,normal])
...
fit=fit_signatures(counts=colon_counts[paste0(patient,'_cancergene'),],opportunities = "human-exome",signatures = final_sigs[ref_sigs_present,],iter = 20000,warmup = 10000,model="poisson",chains = 2)
colon_exposure=retrieve_pars(fit,par = "exposures",hpd_prob = 0.95)
ratios_colon_geno["POLD1_D316N","cancergenes_mean"]=rowSums(colon_exposure$mean[1,ref_sigs_present%in%c(normal,pol)])/rowSums(colon_exposure$mean[1,normal])
ratios_colon_geno["POLD1_D316N","cancergenes_lower95"]=rowSums(colon_exposure$lower_95[1,ref_sigs_present%in%c(normal,pol)])/rowSums(colon_exposure$lower_95[1,normal])
ratios_colon_geno["POLD1_D316N","cancergenes_upper95"]=rowSums(colon_exposure$upper_95[1,ref_sigs_present%in%c(normal,pol)])/rowSums(colon_exposure$upper_95[1,normal])

POLE_botseq_exposures_mean=read.table("~/Desktop/Phil_POL/POLE_botseq_exposures_mean.txt",sep="\t")
POLD_botseq_exposures_mean=read.table("~/Desktop/Phil_POL/POLD_botseq_exposures_mean.txt",sep="\t")

wg_means=c(ratios_colon_geno["POLE","all_mean"], 
           1/rowSums(endo_exposures$mean[1,c("SBS1","SBS5")]),
           1/rowSums(POLE_botseq_exposures$mean[2:1,c("SBS1","SBS5")]),
           1/rowSums(skin_exposures$mean[1,c("SBS1","SBS5","SBS7a","SBS7b")]),
           1/rowSums(POLD_botseq_exposures_mean[2,c("SBS1","SBS5")]),
           ratios_colon_geno["POLD1_S478N","all_mean"],
           1/rowSums(POLD_botseq_exposures_mean[1,c("SBS1","SBS5")]),
           ratios_colon_geno[c("POLD1_L474P","POLD1_D316N"),"all_mean"])

#Fig4A
pdf("~/Desktop/Phil_POL/wg_increase.pdf",useDingbats = F)
...
#Fig4B
pdf("~/Desktop/Phil_POL/coding_increase.pdf",useDingbats = F,height=6,width=5)
